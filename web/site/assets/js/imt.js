...
/* Slider for AnnMenuItems, sorted by AnnMenuHead: requires mootools fx.slide */
function SlideAnnMenuItems() {
  var AnnMenuItemCount = 1;
  $$('li.AnnMenuHead').each(function(el) {
    el.setProperty('id',AnnMenuItemCount);
    SlideDiv = el.getNext('div').setProperty('id','vertical_slide_'+AnnMenuItemCount);
    var AnnMenuItemSlide = new Fx.Slide('vertical_slide_'+AnnMenuItemCount).hide();
    el.addEvent('click', function(e) {
      e.stop();
      AnnMenuItemSlide.toggle();
    });
    AnnMenuItemCount++;
  });
}
...
//  TheArea.scrollIntoView(); //this is unreliable; better option is below.
  ...
  // Slidein the current AnnMenuItemList to view the Menu; mootools required!
  if ( typeof TheMenuItem.parentNode.getProperty === "function" ) {
    var SliderItem = TheMenuItem.parentNode.getProperty('id');
    var AnnMenuItemSlide = new Fx.Slide(SliderItem).slideIn();
